	...
	(function ( $ ) {
		$.fn.jSlidingBanner = function(options) {
			...
			function swapBannerImages() {
			    var allImages = $(".sliding-banner-container .sliding-banner img");
			    for (var i = 0; i < allImages.length; i++) {
			        var imgSource = $(allImages[i]);
			        var bigImage = imgSource.attr("data-src-big");
			        var smallImage = imgSource.attr("data-src");
			        var resWidth = window.innerWidth || document.documentElement.clientWidth;
			        if (resWidth < 768) {
			            imgSource.attr("src", smallImage);
			        }
			        else {
			            imgSource.attr("src", bigImage);
			        }
			    }
			}
			swapBannerImages();
			...
			function setNavIcons()
			{
				...
				$(sliderItemClass).each(function(index){
				    $img = '<img data-index="' + index + '" class="sliding-banner-nav-icon" src="/SiteAssets/images/hole_circle.png">';
					if(currentImage==index)
					{
					    $img = '<img data-index="' + index + '" class="sliding-banner-nav-icon" src="/SiteAssets/images/full_circle.png">';
					}
					$nav.append(
						$img				
						);
